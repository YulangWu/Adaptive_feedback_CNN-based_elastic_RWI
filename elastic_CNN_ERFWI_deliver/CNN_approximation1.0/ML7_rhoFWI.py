...
from ML7_parse_rho import *
from ML7_pre_post_process_rho import *
from ML7_buildmodel_rho import *

import shutil

def main():
    os.environ["CUDA_VISIBLE_DEVICES"]="0"
    a.parameter_dir = a.parameter_dir + '_rho'
    ...
    rho_init = (rho_init - a.mean_rho) / a.max_rho
    rho_true = (rho_true - a.mean_rho) / a.max_rho
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(a.max_epochs):
            ...
            for index in index_arr:
                ...
                input_rho_init = rho_init[[index_arr[index]],:]
                input_rho_true = rho_true[[index_arr[index]],:]


                input_image_vz.shape = a.nx,a.nz
                input_image_vx.shape = a.nx,a.nz
                input_rho_init.shape = a.nx,a.nz
                input_rho_true.shape = a.nx,a.nz


                if a.mode == 'train' or a.mode == 'export':
                    results = sess.run(fetches,
                                    feed_dict={inputs: input_image_vz, inputs2: input_rho_init, inputs3: input_image_vx, targets: input_rho_true})
                else:
                    zero_data = np.zeros((a.nx,a.nz))
                    results = sess.run(fetches,
                                    feed_dict={inputs: input_image_vz, inputs2: input_rho_init, inputs3: input_image_vx, targets: zero_data})
                ...
                if a.figs and (a.max_epochs == 1 or (epoch + 1) % a.display_freq == 0):
                    ...
                    if index % int(len(input_image_vz)/int(len(input_image_vz))) == 0:
                        ...
                        output_res[1,:,:] = input_rho_init[:,:]*a.max_rho + a.mean_rho
                        output_res[2,:,:] = input_rho_true[:,:]*a.max_rho + a.mean_rho

                        output_res[3,:,:] = results['outputs'][-1][0,:,:,0]*a.max_rho + a.mean_rho
                        ...
                        filename = os.path.join(a.output_dir, a.mode + str(index_arr[index]) + '_rho.dat')
            ...
            if should(epoch, a.store_weights_freq) and a.mode =='train':
                ...
                loss_filename = os.path.join(a.output_dir, 'L2_loss_' + a.mode + '_rho.dat')
