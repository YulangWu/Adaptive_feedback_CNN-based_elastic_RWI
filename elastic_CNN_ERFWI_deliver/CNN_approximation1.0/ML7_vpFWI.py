            ...
            for index in index_arr:
                ...
                if a.mode == 'train' or a.mode == 'export':
                    results = sess.run(fetches,
                                    feed_dict={inputs: input_image_vz, inputs2: input_vp_init, inputs3: input_image_vx, targets: input_vp_true})
                else:
                    zero_data = np.zeros((a.nx,a.nz))
                    results = sess.run(fetches,
                                    feed_dict={inputs: input_image_vz, inputs2: input_vp_init, inputs3: input_image_vx, targets: zero_data})
                ...
                if a.figs and (a.max_epochs == 1 or (epoch + 1) % a.display_freq == 0):
                    ...
                    if index % int(len(input_image_vz)/int(len(input_image_vz))) == 0:
                        ...
                        output_res[1,:,:] = input_vp_init[:,:]*a.max_vp + a.mean_vp
                        output_res[2,:,:] = input_vp_true[:,:]*a.max_vp + a.mean_vp
