...
open(3, file='./given_models0/'//trim(adjustl(filenumber_iteration))//trim(adjustl(truevp))//'vp.dat',status='old'& 
...
open(3, file='./given_models0/'//trim(adjustl(filenumber_iteration))//trim(adjustl(truevp))//'vs.dat',status='old'& 
,form='formatted', access='direct', recl=17,iostat=rc)
if(rc/=0) print *,'open file failed!'
do i = 1,nx
do j = 1,nz
...
open(3, file='./given_models0/'//trim(adjustl(filenumber_iteration))//trim(adjustl(truevp))//'rho.dat',status='old'& 
,form='formatted', access='direct', recl=17,iostat=rc)
if(rc/=0) print *,'open file failed!'
do i = 1,nx
do j = 1,nz
...
open(3, file='./given_models0/'//trim(adjustl(filenumber_iteration))//trim(adjustl(initvp))//'vp.dat',status='old'& 
,form='formatted', access='direct', recl=17,iostat=rc)
if(rc/=0) print *,'open file failed!'
do i = 1,nx
do j = 1,nz
...
open(3, file='./given_models0/'//trim(adjustl(filenumber_iteration))//trim(adjustl(initvp))//'vs.dat',status='old'& 
,form='formatted', access='direct', recl=17,iostat=rc)
if(rc/=0) print *,'open file failed!'
do i = 1,nx
do j = 1,nz
...
open(3, file='./given_models0/'//trim(adjustl(filenumber_iteration))//trim(adjustl(initvp))//'rho.dat',status='old'& 
,form='formatted', access='direct', recl=17,iostat=rc)
if(rc/=0) print *,'open file failed!'
do i = 1,nx
do j = 1,nz
...
! open(4, file='./output0/vp.dat')
! close(4,status='delete')
! open(4, file='./output0/vp_init.dat',status='new'& 
! ,form='formatted', access='direct', recl=17,iostat=rc)
! if(rc/=0) print *,'open file failed!'
! do i = -2,nx+3
! do j = -2,nz+3
...
! open(5, file='./output0/rho_init.dat')
! close(5,status='delete')
! open(5, file='./output0/rho.dat',status='new'& 
! ,form='formatted', access='direct', recl=17,iostat=rc)
! if(rc/=0) print *,'open file failed!'
! do i = -2,nx+3
! do j = -2,nz+3
...
do ixs = 5,nx-4,incre_s
    ...
    !   Compute stresses and particle velocities:
        ...
        if(mod(it,it_display) == -1)then
            ...
            filename = './output0/forwavefield_vz'//trim(adjustl(filenumber_iteration))//'.dat'
            ...
            write(filenumber_iteration,'(i5)')it
            filename = './output0/forwavefield_vx'//trim(adjustl(filenumber_iteration))//'.dat'
    ...
    !   Compute stresses and particle velocities:
        ...
        if(mod(it,it_display) == -1)then
            write(filenumber_iteration,'(i5)')it
            filename = './output0/wavefield_vz_Dwave_only'//trim(adjustl(filenumber_iteration))//'.dat'
            open(110, file=filename)
            close(110,status='delete')
            open(110, file=filename,status='new'& 
            ,form='formatted', access='direct', recl=17,iostat=rc)
            ...
            write(filenumber_iteration,'(i5)')it
            filename = './output0/wavefield_vx_Dwave_only'//trim(adjustl(filenumber_iteration))//'.dat'
            open(111, file=filename)
            close(111,status='delete')
            open(111, file=filename,status='new'& 
            ,form='formatted', access='direct', recl=17,iostat=rc)
    ...
    ! open(10, file='./output0/trueseismogram_vz'//trim(adjustl(filenumber_iteration))//'.dat')
    ! close(10,status='delete')
    ! open(10, file='./output0/trueseismogram_vz'//trim(adjustl(filenumber_iteration))//'.dat',status='new'& 
    ! ,form='formatted', access='direct', recl=17,iostat=rc)
    ! if(rc/=0) print *,'open file failed!'

    ! open(11, file='./output0/trueseismogram_vx'//trim(adjustl(filenumber_iteration))//'.dat')
    ! close(11,status='delete')
    ! open(11, file='./output0/trueseismogram_vx'//trim(adjustl(filenumber_iteration))//'.dat',status='new'& 
    ! ,form='formatted', access='direct', recl=17,iostat=rc)
    ! if(rc/=0) print *,'open file failed!'
    ! do i = 1,nx
    !     do j = 1,nt
    ...
    !   Compute stresses and particle velocities:
        ...
        if(mod(it,it_display) == -1)then
            write(filenumber_iteration,'(i5)')it
            filename = './output0/backwavefield_vz'//trim(adjustl(filenumber_iteration))//'.dat'
            open(110, file=filename)
            close(110,status='delete')
            open(110, file=filename,status='new'& 
            ,form='formatted', access='direct', recl=17,iostat=rc)
            ...
            write(filenumber_iteration,'(i5)')it
            filename = './output0/backwavefield_vx'//trim(adjustl(filenumber_iteration))//'.dat'
            open(111, file=filename)
            close(111,status='delete')
            open(111, file=filename,status='new'& 
            ,form='formatted', access='direct', recl=17,iostat=rc)
    ...
    ! write(filenumber_iteration,'(i5)')ixs
    ! open(10, file='./output0/image_vz'//trim(adjustl(filenumber_iteration))//'.dat')
    ! close(10,status='delete')
    ! open(10, file='./output0/image_vz'//trim(adjustl(filenumber_iteration))//'.dat',status='new'& 
    ! ,form='formatted', access='direct', recl=17,iostat=rc)
    ! if(rc/=0) print *,'open file failed!'

    ! open(11, file='./output0/image_vx'//trim(adjustl(filenumber_iteration))//'.dat')
    ! close(11,status='delete')
    ! open(11, file='./output0/image_vx'//trim(adjustl(filenumber_iteration))//'.dat',status='new'& 
    ! ,form='formatted', access='direct', recl=17,iostat=rc)
    ! if(rc/=0) print *,'open file failed!'
